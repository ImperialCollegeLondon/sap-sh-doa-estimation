% demo_pwd_srp loads a spherical microphone array recording and uses plane
% wave decomposition beamforming to estimate the directions of arrival
%
...
tic
[est_doa_vec,srp,azg,incg] = est_doas_pwdsrp(z,fs,mic,params);
elapsed = toc;
fprintf('PWDSRP processing done in %2.1f seconds\n',elapsed);
...
figure;
map = 10*log10(srp./max(srp(:)));
[~, cax] = plot_2D_map_with_est_gt_doa(azg,incg,map,est_doa_vec,gt_doa_vec,contour_levels);
ylabel(cax,'Normalised PWD-SRP [dB]')
set(gca,'clim',[-12 0])
title('Plane-wave decomposition steered response power with estimated and true DOAs')
